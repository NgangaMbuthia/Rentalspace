...
use Modules\Backend\Entities\LandlordTransaction;
...
class ProviderController extends Controller
{
    ...
    public function getLandloadMonthlyDue()
    {
       $data['page_title']="Monthly Due";
        $data['months']=array("Jan"=>"01","Feb"=>"02","Mar"=>"03","Apr"=>"04","May"=>"05","Jun"=>"06","Jul"=>"07","Aug"=>"08","Sep"=>"09","Oct"=>10,"Nov"=>11,"Dec"=>12);


      return view('backend::provider.landload_due',$data);

    }

    public function fetchmlPayments(Request $request)
    {
       $data=$request->all();


         return view('backend::provider._landloaddue',$data);
          ...
          $results = $reader->get()->toArray();
            $numbers=array();
             ...
             {\DB::beginTransaction();
                ...
                $property=Property::where('title',$result['property'])->first();
                  
                 
                 if($property)
                 {
                  
                   $model=Space::where(['property_id'=>$property->id,'number'=>$result['unit_name'],'floor'=>$result['floor']])->first();
                     if(!$model)
                     {
                      $model=new Space();
                     }
                    
                    $space_status=(strlen($result['status'])<1)?"Occupied":$result['status'];
                    $status=$space_status;
                     if($status=="Landload" || $status=="caretaker") 
                          {
                            $occupants=$status;
                          }  else{
                            $occupants="Tenant";
                  ...
                  $model->template_id=null;
                    ...
                    $model->floor=$result['floor'];
                    $model->number=$result['unit_name'];
                     $model->current_occupant=$occupants;
                    
                    $model->status=($result['status']=="Vaccant")?"Free":$result['status'];
                    ...
                    $model->save();
                      ...
                      if(preg_match('/OCCUPIED/i', $model->status) )
                      {
                        
                           if($model->current_occupant!="caretaker" || $model->current_occupant!="landload")
                           {
                         ...
                         $tenant_charges=array("Rent"=>$tenant->monthly_fee,"Deposit"=>$result['deposit'],"Gabbage"=>$result['gabbage'],'Water'=>$result['water']);
 ...
 \DB::commit();
    ...
    public function createTenantUser($result)
    {
       ...
       $user->name=(strlen($result['tenant_name']))?$result['tenant_name']:$faker->name;
       $user->email=null;
       $user->username=null;
        ...
        $data['model']=new PropertyTransaction();
          if($request->isMethod("post"))
          {
             $data=$request->all();
                ...
                $model=Property::find($data['property_id']);
             ...
             DB::beginTransaction();
              ...
              $transaction->other_details=$data['otherDetails'];
              ...
              $account=PropertyAccount::where(['provider_id'=>$p_id,'property_id'=>$data['property_id'],'account_type'=>'Credit'])->first();
                 if(!$account)
                 {
                   ...
                   $account->provider_id=$p_id;
                   $account->property_id=$data['property_id'];
              ...
              $transaction->save();
               ...
               $latestLandload=LandlordTransaction::where(['landlord_user_id'=>$model->landlord_user_id])->latest('id')->first();
                    if($latestLandload)
                    {
                      $old_balance=$latestLandload->new_balance;
                    }else{
                      $old_balance=0;
                    }
                $lmodel=new LandlordTransaction();
                $lmodel->landlord_user_id= $model->landlord_user_id;
                $lmodel->property_id=$model->id;
                $lmodel->transaction_date=date('Y-m-d');
                $lmodel->credit=$transaction->credit;
                $lmodel->debit=0;
                $lmodel->transaction_id=$transaction->id;
                $lmodel->old_balance=$old_balance;
                $lmodel->new_balance=$lmodel->old_balance+$lmodel->debit-$lmodel->credit;
                $lmodel->user_id=auth::user()->id;
                $lmodel->save();
 ...
 Helper::sendEmail($provider->email,$body,"Account Activation Key");
  ...
  public function createUtility($model,$utilities)
  {
   ...
   public function createAmentities($model,$amenities){
    ...
    public function EditCommisiion($id,Request $request)
    {
      $model=Property::find($id);
         if(!$model)
         {
          ...
          $data['url']=url()->current();
            ...
            {
               $this->validate($request,[
                'agent_commission_percentage'=>'required|integer|between:0,100'
               ]);

               $data=$request->all();

               $model->agent_commission_percentage=$data['agent_commission_percentage'];
               $model->save();
               Session::flash("success_msg","Agent Commission Updated Successfully");
               return redirect()->back();
               
            }

           return view('backend::properties._comm',$data);
         }

    }


    public function getLandloadCulumative($year,Request $request)
    {
       $data=$request->all();
        $month=$data['month'];
        ...
        $id=Auth::User()->getProvider->id;
            $models=\DB::select(\DB::raw("call getLandlordAmount($id,$month,$year)"));
    ...
    public function fetchProperties($type=null){
       if(Entrust::hasRole("Admin")){
        ...
        $id=Auth::User()->getProvider->id;
            $models=\DB::select(\DB::raw("call getProperties($id)"));
          $models=collect($models);
             ...
             ->addColumn('action', function ($model) {
                     ...
                     $charge_url=url('/backend/tenants/AddTenantCharges/'.$model->id);
                     $repair_url=url('/backend/repair/index/'.$model->id);
                      $edit_url=url('/backend/property/Comm/'.$model->id);
                      return '
                                <div class="dropdown">
                              <button class="btn btn-xs btn-info dropdown-toggle" aria-expanded="true" type="button" data-toggle="dropdown">Actions
                              <span class="caret"></span></button>
                              <ul class="dropdown-menu">
                                <li><a href="'.$url.'">Detailed View</a></li>
                                <li><a href="'.$url_to.'">Update Details</a></li>
                                <li><a href="#" class="reject-modal" data-url="'.$url_manager.'" data-title="Properety Managed By">View Caretakers</a></li>
                                 <li><a href="#" class="reject-modal" data-url="'.$edit_url.'" data-title="Edit Agent Commission">Edit Commission</a></li>
                                ...
                                <li><a class="reject-modal" data-title="Add New Charges To Current Tenants" data-url="'.$charge_url.'">Add Tenants Charge</a></li>
    ...
    */
    ...
    public function fetchSpaceTenants($value=''){
      ...
      $models=Tenant::join('users','users.id','=','tenants.user_id')
             ...
             ->where(['spaces.property_id'=>$value]);
           return Datatables::of($models)
              ->editColumn('created_at',function($model){
    ...
    public function fetchUnitsAll($status){
       if($status=="All"){
        $models=Space::select(['spaces.id','properties.title','number','spaces.title as name','spaces.currency','spaces.unit_price','spaces.status','spaces.floor','water_meter_number','electricity_meter_number'])->join('properties','properties.id','=','spaces.property_id')->where(['provider_id'=>Auth::User()->getProvider->id]);
         ...
         {
          $models=Space::select(['spaces.id','properties.title','number','spaces.title as name','spaces.currency','spaces.unit_price','spaces.status','spaces.floor','water_meter_number','electricity_meter_number'])->join('properties','properties.id','=','spaces.property_id')->where(['provider_id'=>Auth::User()->getProvider->id])
          ->whereIn('spaces.status',array("Free","Empty","Vaccant","VACANT","VACANTT"));
    ...
    protected function getYears($property_id=null){
      ...
      {
        $data['properties']=Property::where(['provider_id'=>auth::user()->getProvider->id])->OrderBy('title')->get();
         ...
         if($request->isMethod('post'))
         {
           ...
           $models=\DB::select(\DB::raw("call getMonthlyBreakdown($property_id,'$month','$year')"));;
            ...
            try{
              ...
              MonthlySummary::generateBreakdown($models,$data,$advance, $property);
