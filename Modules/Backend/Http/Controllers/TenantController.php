          ...
          ->sum('amount_paid');
           ...
           if($request->isMethod("post"))
           {
              ...
              $garbage=$data['garbage'];
                 ...
                 {
                     if($rent[$key]>0 && strlen($rent[$key])>0)
                     {
                      $tenant=Tenant::find($tenants[$key]);
                        if($tenant)
                        {
                          ...
                          $rentAmount=(strlen($rent[$key])>0)?$rent[$key]:0;
                          $waterAmount=(strlen($water[$key])>0)?$water[$key]:0;
                          $garbageAmount=(strlen($garbage[$key])>0)?$garbage[$key]:0;
              ...
              $summary=\App\TenantMonthlyReport::where(['tenant_id'=>$tenant->id,'month'=>date("M"),'year'=>date('Y')])->latest('id')->first();
               $summary->invoice_amount=$summary->new_balance=$amountdue;
               
               $summary->amount_paid=$summary->amount_paid+$amount;
               $summary->balance=$summary->invoice_amount-$summary->amount_paid;
               $summary->deposit=$deposit[$key];
               
               $summary->rent=$summary->rent+$rentAmount;
               $summary->water_bill=$summary->water_bill+$waterAmount;
               $summary->gabbage_bill=$summary->gabbage_bill+$garbageAmount;
               $property=$tenant->space->property;
               $summary->over_paid_by=($summary->balance<0)?abs($summary->balance):0;
               $percentage=$property->agent_commission_percentage;
                if(strlen($percentage)<0)
                {
                  $landload=(100)/100;
                }else{
                  $landload=(100-$percentage)/100;
                }
                $landload_amount=$landload*$rentAmount;
                $agent_amount=$rentAmount-$landload_amount;
                $summary->direct_amount=$summary->direct_amount+$directAmount;

                $summary->landload_amount=$summary->landload_amount+$landload_amount;
                $summary->agent_commision=$summary->agent_commision+$agent_amount;
                $summary->save();
                ...
                $lmodel->credit=0;
                $lmodel->debit=$summary->landload_amount;
                ...
                $newsummary=TenantSummary::where(['tenant_id'=>$tenant->id,'month'=>date("M"),'year'=>date('Y')])->latest()->first();
                          
                          $newsummary->outstanding_balance=$newsummary->bal_brought_forward+$newsummary->invoice_amount;
                          $newsummary->amount_paid=$newsummary->amount_paid+$payment->debit;
                           $newsummary->new_balance=$newsummary->outstanding_balance-$newsummary->amount_paid;
                           $newsummary->bal_carried_foward=$newsummary->new_balance;

                        
                           if($newsummary->bal_carried_foward==0)
                             {
                              $newsummary->remarks="No Balance";
                             }else if($summary->bal_carried_foward>0)
                             {
                              $newsummary->remarks="Has Balance";
                             }else{
                                $newsummary->remarks="has OverPaid";
                             }
                             $newsummary->save();
                    $invoices=Invoice::where(['space_id'=>$tenant->space_id,'status'=>'Pending'])->get();
                           ...
                           foreach($invoices as $invoice)
                           {
                              ...
                              if($amount>=$invoice->amount)
                            {
                              ...
                              $invoice->amount_paid=$payment->debit;
                              $myamount=$payment->debit;
                              $invoice->balance=$invoice->amount-$invoice->amount_paid;
                              $invoice->save();
                              
                              $this->checkHalfInvoices($invoice,$tenant,$payment->debit);
                            }
                          }
                          DB::commit();
                                ...
                                if(preg_match('/water/i', $item->name) && $amount>0 && $item->balance>0 )
                                {
                                 ...
                                 $summary=\App\TenantMonthlyReport::where(['tenant_id'=>$tenant->id,'month'=>date("M"),'year'=>date('Y')])->latest('id')->first();
                                    if($summary)
                                    {
                                      ...
                                      $item->balance=$item->amount-$item->amount_paid;
                                      $item->save();
                                       $amount=$amount-$summary->water_bill;
                                 ...
                                 if(preg_match('/gabb/i', $item->name)&& $amount>0 && $item->balance>0)
                                {
                                 ...
                                 $summary=\App\TenantMonthlyReport::where(['tenant_id'=>$tenant->id,'month'=>date("M"),'year'=>date('Y')])->latest('id')->first();
                                    if($summary)
                                    {
                                      ...
                                      $item->balance=$item->amount-$item->amount_paid;
                                      $item->save();
                                      $amount=$amount-$summary->gabbage_bill;
